...
GENERAL_AGENT_PROMPT = (
    ...
    "exists), pick the right layer, inspect before you act, and verify the result before reporting.\n\n"
...
LINKEDIN_AGENT_PROMPT = GENERAL_AGENT_PROMPT + (
    ...
    "every step by observing the result, never assuming a click worked. DEFAULT TO COMPLETENESS AND ACCURACY "
    "OVER SPEED: prefer the richest mode and capture every useful field, confirm a profile's real state rather "
    "than guessing, and treat a smaller fully-detailed, verified result as better than a large shallow one.\n\n"
    ...
    "  - linkedin-messages: consumes profile_url and messages ONLY 1st-degree connections. A per-row 'message' "
    "column personalizes per recipient (overrides the message/messages params); 'messages' alternates several. "
