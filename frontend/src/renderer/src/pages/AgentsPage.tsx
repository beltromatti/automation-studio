...
export default function AgentsPage() {
  ...
  return (
    <>
            ...
            return (
              <div key={a.id} className="card card-hover p-5 flex flex-col">
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t" style={{ borderColor: "var(--color-line)" }}>
                  <span className="text-[11px] text-faint mono">{last ? `last ${last.status}` : "never launched"}</span>
                  <div className="flex items-center gap-1.5">
                    ...
                    <Link to={`/agents/${a.id}`} className="btn btn-primary btn-sm">
      ...
      {editing && <AgentEditor agent={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); loadAgents(); }} />}
    </>
  );
}

function AgentEditor({ agent, onClose, onSaved }: {
  agent: AgentDef | "new"; onClose: () => void; onSaved: () => void;
}) {
  ...
  const save = async () => {
    ...
    try {
      const body = { name, description, systemPrompt, scopes: scopes.includes("studio") ? scopes : ["studio", ...scopes] };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "#000000aa" }} onClick={onClose}>
      <div className="card p-5 overflow-auto" style={{ width: 580, maxWidth: "92vw", maxHeight: "90vh" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="text-[11px] text-faint mb-3 flex items-center gap-1.5"><Icon name="sparkles" size={12} /> The engine (Codex / Claude) is picked when you launch a session, not here.</div>
